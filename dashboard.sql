...
with abc as (
    select
        ...
        count(distinct l.lead_id) filter (where amount > '0') as purchases_count
    from sessions as s
...
select
    ...
    utm_campaign,
    utm_medium,
    coalesce(round(sum(total_cost) / sum(leads_count), 2), 0) as cpl,
    coalesce(round(sum(total_cost) / sum(purchases_count), 2), 0) as cppu,
    coalesce(round(sum(total_cost) / sum(visitors_count), 2), 0) as cpu,
    round((sum(revenue) - sum(total_cost)) / sum(total_cost) * 100.00, 2) as roi
from agregation
